...
class AuthenticationClient(APIClient):
	...
	def login(self, request: LoginRequestSchema) -> LoginResponseSchema:
		...
		# return LoginResponseSchema(**response.json()) #  если ответом придет не json, а что-то что нельзя сериализовать, то будет ошибка
		# Инициализируем модель через валидацию JSON строки. Валидация на уровне pydantic
